...
export default function Home(){
 ...
 return <>
  ...
  <section className="card hero"><h2>Calendrier des matchs</h2><p className="muted">Préparez librement les matchs. Ouvrez les votes à la fin du match, puis clôturez-les quand tout le monde a voté.</p>{matches.length===0&&<p>Aucun match préparé.</p>}{[...matches].sort((a,b)=>new Date(a.played_at).getTime()-new Date(b.played_at).getTime()).map(m=><div className="matchRow" key={m.id}><div><b>{m.label||"Match sans nom"}</b><div className="muted">{new Date(m.played_at).toLocaleDateString("fr-BE")} · {m.status==="scheduled"?"À venir":m.status==="open"?"Votes ouverts":"Clôturé"} · {m.ballot_count} vote(s)</div></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{m.status==="scheduled"&&!openMatch&&<button className="btn" onClick={()=>open(m.id)}>Ouvrir les votes</button>}{<button className="btn danger" onClick={()=>removeScheduled(m.id,m.label)}>Supprimer</button>}{m.status==="open"&&<button className="btn danger" onClick={close}>Clôturer les votes</button>}{m.status==="closed"&&!openMatch&&<button className="btn secondary" onClick={()=>reopen(m.id)}>Rouvrir le match</button>}</div></div>)}</section>
