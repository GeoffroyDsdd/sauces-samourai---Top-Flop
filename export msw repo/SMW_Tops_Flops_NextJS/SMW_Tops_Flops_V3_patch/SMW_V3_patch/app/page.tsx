...
export default function Home(){
 ...
 async function prepare(){
  const{error}=await supabase.rpc("create_scheduled_match",{p_label:label,p_played_at:date||null});
 ...
 async function vote(e:React.FormEvent){e.preventDefault();if(!openMatch)return;let token=localStorage.getItem("smw-voter-token");if(!token){token=crypto.randomUUID()+crypto.randomUUID();localStorage.setItem("smw-voter-token",token)}const{error}=await supabase.rpc("submit_ballot",{p_match_id:openMatch.id,p_voter_token:token,p_top1:f.top1,p_top1_comment:f.c1,p_top2:f.top2,p_top2_comment:f.c2,p_top3:f.top3,p_top3_comment:f.c3,p_flop:f.flop,p_flop_comment:f.cf});if(error)setMsg(error.message);else{setSent(true);setF({});load()}}
 ...
 return <>
  ...
  <section className="card"><h3>Ajouter un match au calendrier</h3><p className="muted">Le nom est entièrement libre. Seule la date est affichée, sans heure.</p><div className="grid"><input className="input" value={label} onChange={e=>setLabel(e.target.value)} placeholder="Écrivez le nom que vous voulez"/><input className="input" type="date" value={date} onChange={e=>setDate(e.target.value)}/></div><button className="btn secondary" onClick={prepare}>Ajouter le match</button></section>
  ...
  {openMatch?<form className="card hero" onSubmit={vote}><h2>Vote actuel · {openMatch.label}</h2><label>Top 1 (+3)</label>{choice("top1")}<textarea required value={f.c1||""} placeholder="Pourquoi ce Top 1 ?" onChange={e=>setF({...f,c1:e.target.value})}/><label>Top 2 (+2)</label>{choice("top2")}<textarea required value={f.c2||""} placeholder="Pourquoi ce Top 2 ?" onChange={e=>setF({...f,c2:e.target.value})}/><label>Top 3 (+1)</label>{choice("top3")}<textarea required value={f.c3||""} placeholder="Pourquoi ce Top 3 ?" onChange={e=>setF({...f,c3:e.target.value})}/><label>Flop (-1, séparé des Tops)</label>{choice("flop")}<textarea required value={f.cf||""} placeholder="Pourquoi ce Flop ?" onChange={e=>setF({...f,cf:e.target.value})}/><button className="btn">Envoyer mon vote</button></form>:<div className="card"><h2>Aucun vote ouvert</h2><p className="muted">Choisissez un match dans la liste pour ouvrir ou rouvrir les votes.</p></div>}
